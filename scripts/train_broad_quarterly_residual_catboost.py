"""Train residual CatBoost for broad quarterly salary forecasting."""
...
from src.modeling import BroadQuarterlyResidualCatBoostTrainer, DEFAULT_CATBOOST_PARAMS  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        type=Path,
        default=SRC_PROJECT_ROOT / "models" / "catboost_broad_quarterly_residual_salary_model.cbm",
    )
    parser.add_argument(
        "--params",
        type=Path,
        default=PROCESSED_DATA_DIR / "catboost_broad_quarterly_residual_params.json",
    )
    parser.add_argument(
        "--metrics",
        type=Path,
        default=PROCESSED_DATA_DIR / "catboost_broad_quarterly_residual_metrics.json",
    )
    parser.add_argument(
        "--feature-importance",
        type=Path,
        default=PROCESSED_DATA_DIR / "feature_importance_broad_quarterly_residual.csv",
    )
    parser.add_argument(
        "--predictions",
        type=Path,
        default=PROCESSED_DATA_DIR / "predictions_broad_quarterly_residual_test.parquet",
    )
    parser.add_argument(
        "--residual-dataset",
        type=Path,
        default=PROCESSED_DATA_DIR / "ml_broad_quarterly_residual_salary_dataset.parquet",
    )
    ...
    result = BroadQuarterlyResidualCatBoostTrainer().train_file(
        ...
        residual_dataset_path=args.residual_dataset,
        params=params,
    )
    test_metrics = result.metrics["metrics_by_split"]["test"]
    residual_mape = test_metrics["residual_catboost"].get("mape")
    baseline_mape = test_metrics["baseline"].get("mape")
    print(
        "broad quarterly residual CatBoost trained: "
        f"residual_catboost_test_mape={residual_mape:.2f}%, "
