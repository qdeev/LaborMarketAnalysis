"""Train residual CatBoost for broad monthly salary forecasting."""
...
from src.modeling import BroadMonthlyResidualCatBoostTrainer, DEFAULT_CATBOOST_PARAMS  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        type=Path,
        default=SRC_PROJECT_ROOT / "models" / "catboost_broad_monthly_residual_salary_model.cbm",
    )
    parser.add_argument(
        "--params",
        type=Path,
        default=PROCESSED_DATA_DIR / "catboost_broad_monthly_residual_params.json",
    )
    parser.add_argument(
        "--metrics",
        type=Path,
        default=PROCESSED_DATA_DIR / "catboost_broad_monthly_residual_metrics.json",
    )
    parser.add_argument(
        "--feature-importance",
        type=Path,
        default=PROCESSED_DATA_DIR / "feature_importance_broad_monthly_residual.csv",
    )
    parser.add_argument(
        "--predictions",
        type=Path,
    ...
    parser.add_argument(
        "--residual-dataset",
        type=Path,
        default=PROCESSED_DATA_DIR / "ml_broad_monthly_residual_salary_dataset.parquet",
    )
    ...
    result = BroadMonthlyResidualCatBoostTrainer().train_file(
    ...
    print(
        "broad monthly residual CatBoost trained: "
